...
export function MessageList({ messages, activeChannel, loading }: MessageListProps) {
  ...
  if (messages.length === 0) {
    ...
    const isActivityChannel = activeChannel?.name === "activity";
    const emptyDescription = isActivityChannel
      ? "GitHub에서 새 푸시가 발생하면 이 채널에 자동으로 기록됩니다."
      : activeChannel?.description ?? "첫 메시지를 남겨보세요.";
